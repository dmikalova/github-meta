...
Sequential migration files scatter schema history across many files, making it hard to see "what is
the schema now?" Atlas provides declarative schema-as-code where the schema definition IS the source
of truth, and migrations are generated automatically by diffing against the live database.
...
- `atlas-schema-workflow`: Declarative schema management using Atlas in CI/CD. Schema changes are
  defined in HCL, diffed against production, and applied automatically during deploy.

### Modified Capabilities

- `reusable-workflow`: Update deno-cloudrun.yaml to include Atlas schema apply step instead of deno
  task migrate
...
- **infrastructure**: Ensure GitHub Actions deploy SA has secretmanager.secretAccessor on database
  secrets
