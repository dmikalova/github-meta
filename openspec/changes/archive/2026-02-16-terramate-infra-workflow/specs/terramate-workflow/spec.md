...
The workflow SHALL be defined as a reusable workflow (`workflow_call`) in the
github-meta repo at `.github/workflows/terramate-apply-all.yaml`, callable from
other repos with `secrets: inherit`.
...
- **THEN** `github-actions-infra` impersonates `tofu-ci` via
